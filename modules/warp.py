...
@torch.no_grad()
def render_projection_from_srcs_fast(src_features, src_depths, tgt_intrinsic, src_intrinsics, src2tgt_transform, src_num):
    ...
    projected_features = torch.zeros(B, 3, *src_depths.shape[1:], device=device)
    projected_features[idx[:, 0], :, idx[:, 2], idx[:, 1]] = cur_fused_features.reshape(-1, 3)[mask, :]

    rendered_tgt_depths = torch.zeros(B, 1, *src_depths.shape[1:], device=device)
    ...
    return merge_depths, merge_feats, extrapolation_mask
