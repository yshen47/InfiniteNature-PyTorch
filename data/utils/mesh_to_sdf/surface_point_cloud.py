...
class SurfacePointCloud:
    ...
    def get_sdf_in_batches(self, query_points, use_depth_buffer=False, sample_count=11, batch_size=1000000,
        ...
        batches = [
            self.get_sdf(points, use_depth_buffer=use_depth_buffer, sample_count=sample_count,
                         return_gradients=return_gradients)
    ...
    def get_voxels(self, voxel_resolution, z_rotation, use_depth_buffer=False, sample_count=11, pad=False, check_result=False):
        query_points = get_raster_points(voxel_resolution, z_rotation)
        sdf = self.get_sdf_in_batches(query_points, use_depth_buffer, sample_count)
        ...
        return voxels, query_points

    def sample_sdf_near_surface(self, known_query_points=None, number_of_points=500000, use_scans=True,
                                sign_method='normal', normal_sample_count=11, min_size=0, return_gradients=False):
        query_points = []
        if known_query_points is None:
        ...
        surface_points = np.array(random.choices(list(self.points), k=surface_sample_count))
        ...
        if known_query_points is None:
            ...
            additional_points = sample_uniform_points_in_unit_sphere(unit_sphere_sample_count)
        else:
            additional_points = known_query_points
        # query_points = []
        query_points.append(additional_points)
...
def create_from_scans(mesh, bounding_radius=1, scan_count=100, scan_resolution=400, calculate_normals=True):
    ...
    return SurfacePointCloud(mesh,
                             ...
                             normals=np.concatenate([scan.normals for scan in scans],
