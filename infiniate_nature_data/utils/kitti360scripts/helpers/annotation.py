...
from infiniate_nature_data.utils.kitti360scripts.helpers.labels import labels, id2label, kittiId2label, name2label
...
class KITTI360Bbox3D(KITTI360Object):
    ...
    def parseOpencvMatrix(self, node):
        ...
        data = node.find('infiniate_nature_data').text.split(' ')
...
class Annotation3D:
    # Constructor
    def __init__(self, labelDir='', sequence='', dynamic_object_only=False):
        self.dynamic_object_only = dynamic_object_only
        labelPath = glob.glob(os.path.join(labelDir, '%s.xml' % sequence))  # train or test
        if len(labelPath) != 1:
            raise RuntimeError(
                '%s does not exist! Please specify KITTI360_DATASET in your environment path.' % labelPath)
        else:
...
class Annotation3DPly:
    ...
    def __init__(self, labelDir='', sequence='', isLabeled=True, isDynamic=False, showStatic=True):
        ...
        # True for training infiniate_nature_data, False for testing infiniate_nature_data
        self.isLabeled = isLabeled
        # True for dynamic infiniate_nature_data, False for static infiniate_nature_data
        self.isDynamic = isDynamic
        # True for inspecting static infiniate_nature_data, False for inspecting dynamic infiniate_nature_data
