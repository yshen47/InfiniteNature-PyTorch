...
def loadOxtsData(oxts_dir, frames=None):
  ''' reads GPS/IMU infiniate_nature_data from files to memory. requires base directory
  ...
  if frames==None:
    ...
    for i in range(len(ts)):
      if len(ts[i]):
        try:
          oxts.append(np.loadtxt(os.path.join(oxts_dir, 'infiniate_nature_data', '%010d.txt'%i)))
        except:
          oxts.append([])
      else:
        oxts.append([])
    ...
    if len(frames)>1:
      ...
      for i in range(len(frames)):
        try:
          oxts.append(np.loadtxt(os.path.join(oxts_dir, 'infiniate_nature_data', '%010d.txt'%k)))
        except:
          oxts.append([])
        k=k+1
    ...
    # no list for single value
      ...
      file_name = os.path.join(oxts_dir, 'infiniate_nature_data', '%010d.txt'%k)
      try:
